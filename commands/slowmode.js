...
.addCommand({name: "slowmode",
  ...
  process: async (msg, suffix) => {
    try {
      if (suffix.toLowerCase() == "off" || suffix == "0") {
        msg.channel.edit({rateLimitPerUser: 0}).catch(e => u.errorHandler(e, msg));

        if (molasses.has(msg.channel.id)) clearTimeout(molasses.get(msg.channel.id));
        molasses.delete(msg.channel.id);
      } else {
