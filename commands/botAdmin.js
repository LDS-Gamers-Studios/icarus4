...
.addCommand({name: "ping",
  ...
  process: (msg) => {
    msg.channel.send('Pinging...').then(sent => {
      ...
      if(msg.channel.id != Module.config.channels.botspam && !(msg.member?.roles.cache.has(Module.config.roles.mod) || msg.member?.roles.cache.has(Module.config.roles.management)) u.clean(sent);
    });
  }
})
